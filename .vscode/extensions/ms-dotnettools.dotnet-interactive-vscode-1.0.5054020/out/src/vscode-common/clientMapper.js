    ...
    getOrAddClient(uri) {
        ...
        if (clientPromise === undefined) {
            ...
            clientPromise = new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                try {
                    const { channel, kernelReady } = yield this.config.channelCreator(uri);
                    const config = {
                        channel: channel,
                        createErrorOutput: this.config.createErrorOutput,
                        kernelInfos: (kernelReady === null || kernelReady === void 0 ? void 0 : kernelReady.kernelInfos) || []
                    };
                    ...
                    for (const callback of this.clientCreationCallbacks) {
                        try {
                            ...
                            polyglot_notebooks_1.Logger.default.error(`Error executing client creation callback for ${uri.fsPath}: ${e}`);
                        }
                    }
                    resolve(client);
                }
                ...
                clientPromise.then(client => {
                    for (const callback of this.clientDisposalCallbacks) {
                        try {
                            ...
                            polyglot_notebooks_1.Logger.default.error(`Error executing client disposal callback for ${uri.fsPath}: ${e}`);
                        }
                    }
                    client.dispose();
                });
