...
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const minDotNetSdkVersion = dotnetConfig.get('minimumDotNetSdkVersion') || '8.0';
        ...
        yield (0, commands_1.registerAcquisitionCommands)(context, diagnosticsChannel);
        // check sdk version
        ...
        try {
            ...
            if (!(0, utilities_1.isVersionGreaterOrEqual)(dotnetVersion, minDotNetSdkVersion)) {
                showHelpPage = true;
        ...
        function kernelChannelCreator(notebookUri) {
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const environmentVariables = Object.assign(Object.assign({}, polyglotConfig.get('kernelEnvironmentVariables')), { 'DOTNET_CLI_CULTURE': getCurrentCulture(), 'DOTNET_CLI_UI_LANGUAGE': getCurrentCulture() });
                ...
                const kernelReady = yield channel.waitForReady();
                return {
                    channel,
                    kernelReady
                };
        ...
        This allows adding new cells to the notebook and prompting user for input.`;
            compositeKernel.registerCommandHandler({
                commandType: commandsAndEvents.RequestInputType, handle: (commandInvocation) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    let value;
                    let customInputRequest = yield vscodeNotebookManagement.handleCustomInputRequest(prompt, requestInput.inputTypeHint, password);
                    if (customInputRequest.handled) {
                        ...
                        value = (requestInput.inputTypeHint === "file")
                            ...
                            : yield vscode.window.showInputBox({ prompt, password, ignoreFocusOut: true });
                    }
                    if (!value) {
                        commandInvocation.context.fail('Input request cancelled');
                    }
                    else {
                        commandInvocation.context.publish(new commandsAndEvents.KernelEventEnvelope(commandsAndEvents.InputProducedType, {
                            value
            ...
            compositeKernel.registerCommandHandler({
                commandType: commandsAndEvents.SendEditableCodeType, handle: (commandInvocation) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const succeeded = yield vscodeNotebookManagement.replaceNotebookCells(notebookDocument.uri, range, [newCell]);
                    if (!succeeded) {
        ...
        serviceCollection_1.ServiceCollection.initialize(context, clientMapper, tokensProvider.dynamicTokenProvider);
        context.subscriptions.push(new activeNotebookTracker_1.ActiveNotebookTracker(clientMapper));
        ...
        ////////////////////////////////////////////////////////////////////////////////
        const launchOptions = yield getInteractiveLaunchOptions();
        const serializerCommand = dotnetConfig.get('notebookParserArgs') || [];
        ...
        context.subscriptions.push(serviceCollection_1.ServiceCollection.Instance.KernelInfoUpdaterService.onKernelInfoUpdated((notebook, client) => {
        ...
        context.subscriptions.push(serviceCollection_1.ServiceCollection.Instance.KernelInfoUpdaterService.onKernelInfoUpdated((notebook, client) => {
            ...
            if (activeTextEditor) {
                serviceCollection_1.ServiceCollection.Instance.LanguageConfigurationManager.ensureLanguageConfigurationForDocument(activeTextEditor.document);
            }
        }));
